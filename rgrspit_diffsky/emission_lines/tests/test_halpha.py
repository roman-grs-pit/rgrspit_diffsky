...
import numpy as np
import pytest
from rgrspit_diffsky.emission_lines.halpha import sfr_to_Halpha_KTC94, K98_HALPHA_COEF
...
def test_halpha_luminosity_order_of_magnitude():
    ...
    for i in range(len(test_sfrs)):
        assert min_expected[i] <= log_l_halpha[i] <= max_expected[i], \
            f"H-alpha luminosity for SFR={test_sfrs[i]} M_sun/yr is {log_l_halpha[i]}, " \
